...
def q_update(Q, s, a, reward, s_next, alpha, gamma):
    ...
    temporal_difference_error = reward+gamma*np.max(Q[s_next])-Q[s, a]
    Q[s, a]+=alpha*temporal_difference_error
    return Q
...
def q_learning(Q, env, epsilon, alpha, gamma, epis):
    ...
    for i in range(epis):
        ...
        s, info = env.reset() # observation store the initial state of the environment
        ...
        while True:
            ...
            s_next, r, terminated, truncated, info = env.step(a) # execute the chosen action and observe the outcome
            episode_reward+=r
            ...
            if terminated or truncated:  
                ...
                # truncated: a boolean value indicating whether the episode has been truncated due to conditions outside the scope of the Markov Decison Process, such as exceeding a time limit
                break
...
class DQNAgent:
    ...
    def select_action(self, state):
        ...
        with torch.no_grad():
          if sample>eps_threshold:
            ...
            a = torch.randint(low=0, high=self.n_actions, size=(1, 1), device=self.device, dtype=torch.long)
        return a
    ...
    def get_values(self):
        ...
        if len(self.memory) < self.batch_size:
            return None
        # samples a batch of transitins from memory
        batch = self.memory.sample(self.batch_size)

        # Extracts states, next states, actions, and rewards from the sampled transitions
        next_state = []
        state = []
        action = []
        reward = []
        ...
        # Creates a mask o identify non-final next states
    ...
    def optimize_model(self):
        if self.get_values() is not None:
            ...
            # state_action_values: (batch-size, 1)
        else:
            return
        ...
        # expected_state_action_values: (batch_size, )
...
class PolicyNetwork(nn.Module):
    def __init__(self, number_observation_features: int, number_actions: int):
        ...
        # shape: (batch_size, num_actions) ==> get the action probs for each state
        self.softmax = nn.Softmax(dim=-1) # dim is the dimension for the actions
...
class ReinforceAgent:
    ...
    def choose_action(self, state):
        ...
        action = dist.sample() # action is a tensor
...
def calculate_discounted_returns(episode_rewards, gamma):
    ...
    # Gt = rt+gamma*r_t+1+gamma**2*r_t+2+...
    # Gt = rt+gammaGt+1
    # episode_rewards = [R1, R2, ..., RT]
