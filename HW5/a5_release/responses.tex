...
\author{Due: 5/5/25 11:59 PM on Gradescope}
%UNCOMMENT IF SUBMISSION ALLOWED AFTER LAST DAY OF CLASS
\date{Late submissions accepted until 5/7/25 11:59 PM}
...
\section*{Answer}
Compared to random policy, it is obvious that the Q-learning agent exhibits more structured and goal-directed (the taxi driver do go to pick up passenger and drop off the passenger at the destination). However, the random policy selects actions uniformly at random, resulting in unpredictable trajectories that often fail to reach the goal or take inefficient paths.
...
\section*{Answer}
The DQN agent learns how to balance the cartpole by applying forces in a controlled manner. From the visualization, the agent appears to have learned a policy that keeps the pole upright by continuously adjusting the cart's position. It moves the cart right to counteract the pole's tilt. However, the random policy applies force randomly, causing the pole to quickly lose balance and fall within a few steps.
...
\section*{Answer}
Compared with the DQN images, it is obvious that DQN converges faster than Reinforce, cause DQN achieves higher average rewards and learns more efficiently, while Reinforce shows higer variance and slower improvement. What's more, the policy learned by DQN is more deterministic and exploitative, while Reinforce learns a stochastic policy that maintains exploration even after training. 
